...
// Each site's own favicon, pulled from the live site and re-encoded to a 64px
// webp so the "Visit the live site" link carries the destination's mark rather
// than a generic glyph. Bundled rather than hotlinked: a third-party favicon
// endpoint is a request we do not control on a link the visitor may never click.
// The app MVP is still on a shared github.io host with no icon of its own, so it
// reuses the research site's mark — same project, same team.
import cliftonFavicon from './assets/portfolio/cliftonaveanimalhospital-favicon.webp';
import stoneFavicon from './assets/portfolio/stonebellisimo-favicon.webp';
import nexusFavicon from './assets/portfolio/nexusverium-favicon.webp';
import bodegaFavicon from './assets/portfolio/bodegaproject-favicon.webp';
import stockroomFavicon from './assets/portfolio/stockroomnj-favicon.webp';
import './styles.css';
...
function App() {
  ...
  const submit = async event => {
    ...
    if (!payload.services.length) {
      trackEvent('form_error', { label: 'Start your project', step: 'services', reason: 'missing_service' });
      return setStatus({ text: 'Please select at least one service.', kind: 'error' });
    }
    if (payload.preferredContactMethod === 'phone' && !payload.phone.trim()) {
      trackEvent('form_error', { label: 'Start your project', step: 'phone', reason: 'phone_required_for_contact_method' });
      return setStatus({ text: 'Please include a phone number if you prefer a phone call.', kind: 'error' });
    }
    setStatus({ text: 'Sending…', kind: '' });
    try {
      const leadId = await submitLead(payload, 'intake_form');
      form.reset();
      setSelectedServices([]);
            ...
            <article className="portfolio-story" id="portfolio-dossier" data-section={`portfolio:${projects[activeProject].title}`} aria-hidden={!portfolioPhase.story} inert={!portfolioPhase.story}>
              ...
              <div className="portfolio-story-copy">
                ...
                <a className="portfolio-story-visit" href={projects[activeProject].url} target="_blank" rel="noreferrer" tabIndex={portfolioPhase.story ? 0 : -1}>
                  ...
                  {/* The destination's own favicon does the identifying and the
                      arrow does the "this leaves the page" — the glyph alone was
                      carrying both jobs. Decorative: the link text and the host
                      underneath already name where it goes. */}
                  <i aria-hidden="true">
                    <img src={projects[activeProject].favicon} alt="" width="33" height="33" loading="lazy" decoding="async" />
                    <svg viewBox="0 0 14 14" fill="none" aria-hidden="true">
                      <path d="M4 10 10 4M4.9 4H10v5.1" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round" />
                    </svg>
                  </i>
                </a>
              </div>
